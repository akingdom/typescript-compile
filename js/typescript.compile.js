...
Exposing the translate method Copyright (c) Andrew Kingdom. All rights reserved.
...
if(typeof this.TSCOMPILE_autocompile === 'undefined')
this.TSCOMPILE_autocompile = true;

this.TSCOMPILE = {}

this.TSCOMPILE.hashCode = function(s) {
    var hsh = 0, chr, i;
    if(s.length == 0) {
        return hsh;
    }
...
/**
 * translate
 * @param {string[]} src The Typescript source code strings to be translated.
 * @return {string} A Javascript equivalent source code string.
 */
this.TSCOMPILE.translate = function(src) {
    ...
    // Use cached data if already compiled
    if(window.sessionStorage && sessionStorage.getItem('typescript' + this.hashCode(src.join('')))) {
        outfile.source = sessionStorage.getItem('typescript' + this.hashCode(src.join('')));
    } else {  // not cached
        ...
        // add all script(s) to the compile list
        ...
        if(window.sessionStorage) {
            sessionStorage.setItem('typescript' + this.hashCode(src.join('')), outfile.source);
        }
    }
    
}

this.TSCOMPILE.load = function(url) {
    var xhr;
...
this.TSCOMPILE.compile = function() {
    
    // Collect document's typescript elements as an array of strings
    var script = document.getElementsByTagName('script');
    var body = document.getElementsByTagName('body')[0];
    var i, src = [], elem;
    for(i = 0; i < script.length; i++) {
        if(script[i].type == 'text/typescript') {
            if(script[i].src) {  // Script via URL
                src.push(this.load(script[i].src));
            } else {  // Script content
    ...
    // Add all the compiled scripts to the document as JavaScripts
...
(function() {
    ...
    if this.TSCOMPILE_autocompile
        this.TSCOMPILE.compile();
